...
void
compute (rectangle * one)
{
  float length =
    sqrt (pow ((one->a[1].y - one->a[0].y), 2) +
	  pow ((one->a[1].x - one->a[0].x), 2));
  float breadth =
    sqrt (pow ((one->a[0].y - one->a[2].y), 2) +
	  pow ((one->a[0].x - one->a[2].x), 2));
  one->area = length * breadth;
}

void
compute_n (rectangle a[], int n)
{
  for (int i = 0; i < n; i++)
    compute (&a[i]);
}
